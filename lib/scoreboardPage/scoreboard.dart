...
import 'scoreboard_constants.dart'; // 상수 (API URL, 요일 이름 등)
...
class _ScoreboardScreenState extends State<ScoreboardScreen> {
  ...
  Future<void> _initializeData() async {
    ...
    _dataService = ScoreboardDataService(userId: _userId!); // 사용자 ID로 데이터 서비스 초기화
  ...
  Future<void> _loadDataForCurrentSelection() async {
    ...
    try {
      if (_isSelectedToggle[0]) { // 주간 보기 선택 시
        await _loadDataForWeek(_displayedDate);
      } else if (_isSelectedToggle[1]) { // 월간 보기 선택 시
        await _loadDataForMonth(_displayedDate);
      } else { // 분기별/연간 보기 (현재 미구현 상태)
        if (mounted) {
          setState(() {
  ...
  Future<void> _loadDataForMonth(DateTime targetMonthDate) async {
    ...
    final tempMonthlyDataForAvg = await _dataService.getMonthlyScores(firstDayOfMonth); // 월간 데이터 가져오기
    final averageScore = _dataService.calculateAverageMonthlyScore(tempMonthlyDataForAvg); // 월간 평균 계산
  ...
  void _onPeriodToggleChanged(int index) {
    if (!mounted || _userId == null) return;
    if (_isSelectedToggle[index] && !_isLoading) return; // 이미 선택된 탭이거나 로딩 중이 아니면 변경 없음
    ...
    if (index == 0) { // 주간
      ...
      newDateToDisplay = _displayedDate; // 일단 현재 날짜 유지
       if (mounted) { // 미구현 알림
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text("${_isSelectedToggle[2] ? "분기별" : "연간"} 보기는 아직 준비 중입니다.")),
        );
      }
    }
  ...
  void _switchToWeekViewForDate(DateTime date) {
    if (!mounted || _userId == null) return;
    final newWeekStartDate = date.subtract(Duration(days: date.weekday - 1)); // 선택된 날짜가 속한 주의 월요일
    if (mounted) {
      setState(() {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text("스코어보드", // "Scoreboard" -> "스코어보드"
            style: TextStyle(fontWeight: FontWeight.bold, fontSize: 20)), // 폰트 크기 조정
        centerTitle: true,
        ...
        child: Column(
          ...
          children: [
            ...
            AverageScoreDisplay(
              ...
              onDetailPressed: () { // 상세 보기 버튼 클릭 시 영양 섭취 상세 페이지로 이동
                if (_userId != null) {
                  Navigator.push(
                    context,
              ...
              child: _isLoading // 로딩 중
                  ...
                  : _errorMessage != null // 오류 발생 시
                      ...
                      : _userId == null // 사용자 ID가 없을 때 (보통 로딩 전에 표시됨)
                          ? const Center(child: Text("사용자 정보를 가져오는 중...", style: TextStyle(fontSize: 15, color: Colors.grey)))
                          : _isSelectedToggle[0] // 주간 뷰 선택 시
                              ...
                              : _isSelectedToggle[1] // 월간 뷰 선택 시
                                  ? Container( // 월간 달력 컨테이너
                                      padding: const EdgeInsets.symmetric(vertical: 6.0, horizontal: 2.0), // 패딩 조정
                                      decoration: BoxDecoration(
                                        color: Colors.grey.shade100, // 배경색 약간 변경
                                  ...
                                  // 분기별/연간 보기 (미구현)
                                  : Center(child: Text("${_isSelectedToggle[2] ? "분기별" : "연간"} 뷰는 아직 준비 중입니다.", style: const TextStyle(fontSize: 16, color: Colors.grey))),
            ),
            // 주간 코멘트는 주간 뷰이고, 로딩/에러가 아닐 때, 데이터가 있을 때만 표시
            if (_isSelectedToggle[0] && _currentWeekChartData.isNotEmpty && !_isLoading && _errorMessage == null && _userId != null)
